...
static const size_t g_maxMemory = (sizeof(size_t) == 4) ? (2 *(1U<<30) - 64 *(1 <<20)) : ((size_t)(512 *(1 <<20)) << sizeof(size_t));
                              ...
                              unsigned displayLevel)
{
    ...
    for (fileIndex=0; fileIndex<nbFiles; fileIndex++) {
        ...
        unsigned long long remainingToLoad = (fs64 == ((U64)(-1))) ? 0 : fs64;
        U32 const nbChunks = targetChunkSize ? (U32)((fs64 + (targetChunkSize-1)) / targetChunkSize) : 1;
        U64 const chunkSize = targetChunkSize ? ((targetChunkSize) < (fs64) ? (targetChunkSize) : (fs64)) : fs64;
        size_t const maxChunkSize = (size_t)((chunkSize) < ((128 *(1 <<10))) ? (chunkSize) : ((128 *(1 <<10))));
        U32 cnb;
        FILE* const f = fopen(fileName, "rb");
        if (f==((void *)0)) { if (0) fprintf(stderr, "Error defined at %s, line %i : \n", "dibio.c", 121);; fprintf(stderr, "Error %i : ", 10); fprintf(stderr, "zstd: dictBuilder: %s %s ", fileName, strerror((*__errno_location ()))); fprintf(stderr, "\n"); exit(10); };
        { if (displayLevel>=2) { if ((UTIL_clockSpanMicro(g_displayClock) > g_refreshRate) || (displayLevel>=4)) { g_displayClock = UTIL_getTime(); fprintf(stderr, "Loading %s...       \r", fileName); if (displayLevel>=4) fflush(stderr); } } };
        for (cnb=0; cnb<nbChunks; cnb++) {
            size_t const toLoad = (size_t)((maxChunkSize) < (remainingToLoad) ? (maxChunkSize) : (remainingToLoad));
            if (toLoad > *bufferSizePtr-pos) break;
            {   size_t const readSize = fread(buff+pos, 1, toLoad, f);
                if (readSize != toLoad) { if (0) fprintf(stderr, "Error defined at %s, line %i : \n", "dibio.c", 127);; fprintf(stderr, "Error %i : ", 11); fprintf(stderr, "Pb reading %s", fileName); fprintf(stderr, "\n"); exit(11); };
                pos += readSize;
                ...
                if (toLoad < targetChunkSize) {
                    fseek(f, (long)(targetChunkSize - toLoad), 1);
        }   }   }
        fclose(f);
    }
    if (displayLevel>=2) { fprintf(stderr, "\r%79s\r", ""); };
    *bufferSizePtr = pos;
    if (displayLevel>=4) { fprintf(stderr, "loaded : %u KB \n", (unsigned)(pos >> 10)); }
...
static void DiB_shuffle(const char** fileNamesTable, unsigned nbFiles) {
    U32 seed = 0xFD2FB528;
    unsigned i;
    ((void) sizeof ((nbFiles >= 1) ? 1 : 0), __extension__ ({ if (nbFiles >= 1) ; else __assert_fail ("nbFiles >= 1", "dibio.c", 166, __extension__ __PRETTY_FUNCTION__); }));
                         ...
                         const void* buff, size_t buffSize)
{
    ...
    if (f==((void *)0)) { if (0) fprintf(stderr, "Error defined at %s, line %i : \n", "dibio.c", 216);; fprintf(stderr, "Error %i : ", 3); fprintf(stderr, "cannot open %s ", dictFileName); fprintf(stderr, "\n"); exit(3); };

    { size_t const n = fwrite(buff, 1, buffSize, f);
      if (n!=buffSize) { if (0) fprintf(stderr, "Error defined at %s, line %i : \n", "dibio.c", 219);; fprintf(stderr, "Error %i : ", 4); fprintf(stderr, "%s : write error", dictFileName); fprintf(stderr, "\n"); exit(4); } }

    { size_t const n = (size_t)fclose(f);
      if (n!=0) { if (0) fprintf(stderr, "Error defined at %s, line %i : \n", "dibio.c", 222);; fprintf(stderr, "Error %i : ", 5); fprintf(stderr, "%s : flush error", dictFileName); fprintf(stderr, "\n"); exit(5); } }
}
...
static fileStats DiB_fileStats(const char** fileNamesTable, unsigned nbFiles, size_t chunkSize, unsigned displayLevel)
{
    ...
    for (n=0; n<nbFiles; n++) {
        ...
        U64 const srcSize = (fileSize == ((U64)(-1))) ? 0 : fileSize;
        U32 const nbSamples = (U32)(chunkSize ? (srcSize + (chunkSize-1)) / chunkSize : 1);
        U64 const chunkToLoad = chunkSize ? ((chunkSize) < (srcSize) ? (chunkSize) : (srcSize)) : srcSize;
        size_t const cappedChunkSize = (size_t)((chunkToLoad) < ((128 *(1 <<10))) ? (chunkToLoad) : ((128 *(1 <<10))));
        fs.totalSizeToLoad += cappedChunkSize * nbSamples;
        fs.oneSampleTooLarge |= (chunkSize > 2*(128 *(1 <<10)));
        fs.nbSamples += nbSamples;
    }
    if (displayLevel>=4) { fprintf(stderr, "Preparing to load : %u KB \n", (unsigned)(fs . totalSizeToLoad >> 10)); };
    return fs;
}
                       ...
                       ZDICT_fastCover_params_t* fastCoverParams, int optimize)
{
    ...
    size_t const memMult = params ? 11 :
                           coverParams ? 9:
                           1;
    size_t const maxMem =  DiB_findMaxMem(fs.totalSizeToLoad * memMult) / memMult;
    size_t loadedSize = (size_t) (((unsigned long long)maxMem) < (fs . totalSizeToLoad) ? ((unsigned long long)maxMem) : (fs . totalSizeToLoad));
    void* const srcBuffer = malloc(loadedSize+ 32);
    int result = 0;

     
    if ((!sampleSizes) || (!srcBuffer) || (!dictBuffer))
        { if (0) fprintf(stderr, "Error defined at %s, line %i : \n", "dibio.c", 291);; fprintf(stderr, "Error %i : ", 12); fprintf(stderr, "not enough memory for DiB_trainFiles"); fprintf(stderr, "\n"); exit(12); };    
    if (fs.oneSampleTooLarge) {
        if (displayLevel>=2) { fprintf(stderr, "!  Warning : some sample(s) are very large \n"); };
        if (displayLevel>=2) { fprintf(stderr, "!  Note that dictionary is only useful for small samples. \n"); };
        if (displayLevel>=2) { fprintf(stderr, "!  As a consequence, only the first %u bytes of each sample are loaded \n", (128 *(1 <<10))); };
    }
    if (fs.nbSamples < 5) {
        if (displayLevel>=2) { fprintf(stderr, "!  Warning : nb of samples too low for proper processing ! \n"); };
        if (displayLevel>=2) { fprintf(stderr, "!  Please provide _one file per sample_. \n"); };
        if (displayLevel>=2) { fprintf(stderr, "!  Alternatively, split files into fixed-size blocks representative of samples, with -B# \n"); };
        { if (0) fprintf(stderr, "Error defined at %s, line %i : \n", "dibio.c", 301);; fprintf(stderr, "Error %i : ", 14); fprintf(stderr, "nb of samples too low"); fprintf(stderr, "\n"); exit(14); };    
    }
    if (fs.totalSizeToLoad < (unsigned long long)(8 * maxDictSize)) {
        if (displayLevel>=2) { fprintf(stderr, "!  Warning : data size of samples too small for target dictionary size \n"); };
        if (displayLevel>=2) { fprintf(stderr, "!  Samples should be about 100x larger than target dictionary size \n"); };
    }

     
    if (loadedSize < fs.totalSizeToLoad)
        if (displayLevel>=1) { fprintf(stderr, "Not enough memory; training on %u MB only...\n", (unsigned)(loadedSize >> 20)); };

     
    if (displayLevel>=3) { fprintf(stderr, "Shuffling input files\n"); };
    ...
    {   size_t dictSize;
        if (params) {
            DiB_fillNoise((char*)srcBuffer + loadedSize, 32);    
            ...
            ((void) sizeof ((fastCoverParams != ((void *)0)) ? 1 : 0), __extension__ ({ if (fastCoverParams != ((void *)0)) ; else __assert_fail ("fastCoverParams != NULL", "dibio.c", 339, __extension__ __PRETTY_FUNCTION__); }));
            if (optimize) {
              ...
              if (!ZDICT_isError(dictSize)) {
                ...
                if (displayLevel>=2) { fprintf(stderr, "k=%u\nd=%u\nf=%u\nsteps=%u\nsplit=%u\naccel=%u\n", fastCoverParams->k, fastCoverParams->d, fastCoverParams->f, fastCoverParams->steps, splitPercentage, fastCoverParams->accel); };
              }
            } else {
        ...
        if (ZDICT_isError(dictSize)) {
            if (displayLevel>=1) { fprintf(stderr, "dictionary training failed : %s \n", ZDICT_getErrorName(dictSize)); };    
            result = 1;
            goto _cleanup;
        }
         
        if (displayLevel>=2) { fprintf(stderr, "Save dictionary of size %u into file %s \n", (unsigned)dictSize, dictFileName); };
