...
BMK_runTime_t BMK_extract_runTime(BMK_runOutcome_t outcome)
{
    ((void) sizeof ((outcome . error_tag_never_ever_use_directly == 0) ? 1 : 0), __extension__ ({ if (outcome . error_tag_never_ever_use_directly == 0) ; else __assert_fail ("outcome.error_tag_never_ever_use_directly == 0", "benchfn.c", 74, __extension__ __PRETTY_FUNCTION__); }));
    return outcome.internal_never_ever_use_directly;
}

size_t BMK_extract_errorResult(BMK_runOutcome_t outcome)
{
    ((void) sizeof ((outcome . error_tag_never_ever_use_directly != 0) ? 1 : 0), __extension__ ({ if (outcome . error_tag_never_ever_use_directly != 0) ; else __assert_fail ("outcome.error_tag_never_ever_use_directly != 0", "benchfn.c", 80, __extension__ __PRETTY_FUNCTION__); }));
            ...
            for (blockNb = 0; blockNb < p.blockCount; blockNb++) {
                ...
                if (loopNb == 0) {
                    ...
                    if ((p.errorFn != ((void *)0)) && (p.errorFn(res))) {
                        { { if (0) fprintf(stderr, "%s: %i: \n", "benchfn.c", 143); }; { if (0) fprintf(stderr, "Error : "); }; { if (0) fprintf(stderr, "Function benchmark failed on block %u (of size %u) with error %i", blockNb, (unsigned)p . srcSizes[blockNb], (int)res); }; { if (0) fprintf(stderr, " \n"); }; return BMK_runOutcome_error(res); };
                    }
                    dstSize += res;
            }   }
        }   
...
void BMK_resetTimedFnState(BMK_timedFnState_t* timedFnState, unsigned total_ms, unsigned run_ms)
{
    ...
    timedFnState->timeBudget_ns = (U64)total_ms * (1*1000000000ULL) / 1000;
    timedFnState->runBudget_ns = (U64)run_ms * (1*1000000000ULL) / 1000;
                                  ...
                                  BMK_benchParams_t p)
{
    ...
    while (!completed) {
        ...
        if (UTIL_clockSpanMicro(cont->coolTime) > (70*(1*1000000ULL))) {
            { if (0) fprintf(stderr, "\rcooling down ...    \r"); };
            sleep(10);
        ...
        {   BMK_runTime_t const newRunTime = BMK_extract_runTime(runResult);
            ...
            if (loopDuration_ns > (runBudget_ns / 50)) {
                U64 const fastestRun_ns = ( (bestRunTime . nanoSecPerRun) < (newRunTime . nanoSecPerRun) ? (bestRunTime . nanoSecPerRun) : (newRunTime . nanoSecPerRun) );
                cont->nbLoops = (U32)(runBudget_ns / fastestRun_ns) + 1;
            } else {
                 
                const unsigned multiplier = 10;
                ((void) sizeof ((cont->nbLoops < ((unsigned)-1) / multiplier) ? 1 : 0), __extension__ ({ if (cont->nbLoops < ((unsigned)-1) / multiplier) ; else __assert_fail ("cont->nbLoops < ((unsigned)-1) / multiplier", "benchfn.c", 245, __extension__ __PRETTY_FUNCTION__); }));   
                cont->nbLoops *= multiplier;
            }

            if(loopDuration_ns < runTimeMin_ns) {
                 
                ((void) sizeof ((completed == 0) ? 1 : 0), __extension__ ({ if (completed == 0) ; else __assert_fail ("completed == 0", "benchfn.c", 251, __extension__ __PRETTY_FUNCTION__); }));
                continue;
            } else {
